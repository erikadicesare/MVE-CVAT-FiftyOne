...
def view_prediction(id, pred):
    ...
    if not isExist:
        ...
        info = open("datasets/{}/dataset/info.txt".format(pred), "w")
...
def compare_predictions(id, pred1, pred2):
    ...
    if not isExist and not isExistReverse:
        ...
        # file aggiuntivo usato per scriverci se un dataset ha annotazioni o meno (in caso di confronto gia esistente si va a pescare questa informazine)
        info = open("datasets/{}x{}/dataset/info.txt".format(pred1,pred2), "w")
        ...
        f = open("datasets/{}x{}/dataset/info.txt".format(pred1,pred2), "r")
        hasAnnotations = f.read()

        foIntegration.create_fo_dataset(str(epoch), "datasets/{}x{}/dataset/".format(pred1,pred2), pred1, pred2, hasAnnotations)
        ...
        if not isExist:
            f = open("datasets/{}x{}/dataset/info.txt".format(pred2,pred1), "r")
            hasAnnotations = f.read()

            foIntegration.create_fo_dataset(str(epoch), "datasets/{}x{}/dataset/".format(pred2,pred1), pred1, pred2, hasAnnotations)
...
# prendo i nomi delle immagini che ho caricato su cvat
    ...
    dataset = foIntegration.create_fo_dataset_truth(str(epoch), "datasets/{}xTruth/dataset/".format(pred), pred, hasAnnotations)
